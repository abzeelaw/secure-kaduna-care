...
function Hospitals() {
  ...
  const list = useMemo(() => {
    return data.filter((h) => {
      ...
      const doctorMatch = DEFAULT_DOCTORS.some(
        (doc) => doc.hospital.id === h.id && doc.full_name.toLowerCase().includes(q.toLowerCase()),
      );
      const matchQ =
        !q ||
  ...
  return (
    <PhoneShell>
      ...
      <div className="space-y-3 px-5 pt-4">
        ...
        {list.map((h) => {
          const doctorCount = DEFAULT_DOCTORS.filter((doc) => doc.hospital.id === h.id).length;
          return (
            <Link key={h.id} to={`/hospitals/${h.id}`} className="flex items-center gap-3 rounded-2xl border border-border bg-background p-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="flex items-center gap-2 text-[11px] text-muted-foreground">
                  <span className="flex items-center gap-1">
                    <MapPin className="h-3 w-3" /> {h.lga ?? "Kaduna"}
                  </span>
                  <span>•</span>
                  <span>{h.type}</span>
                </p>
                <p className="mt-1 text-[11px] text-muted-foreground">
                  {doctorCount} registered doctor{doctorCount === 1 ? "" : "s"}
                </p>
